...
def main(input, output, out_res, out_fov, compression_level, 
    ...
    for ipath in input:
        ...
        for plan_episode in plan:
            ...
            # [MODIFICATION FOR TACTILE] a
            for cam_id, cam in enumerate(cameras):
                video_path_rel = cam['video_path']
                video_path_abs = demos_path.joinpath(video_path_rel).absolute()
                video_start, video_end = cam['video_start_end']

                # Check if raw_video.npy exists in the same folder as raw_video.mp4
                npy_path = video_path_abs.parent.joinpath('tactile.npy')
                if npy_path.is_file():
                    print("npy path is file")
                    # Load entire tactile data from .npy
                    tactile_arr = np.load(npy_path, allow_pickle=True)
                    # Slice based on the camera's frame range
                    tactile_slice = tactile_arr[video_start:video_end]

                    # Store in the replay buffer under e.g. camera0_tactile
    ...
    def video_to_zarr(replay_buffer, mp4_path, tasks):
        ...
        if mirror_swap:
            ...
            mirror_mask = draw_predefined_mask(
                mirror_mask, color=(0,0,0), mirror=False, gripper=False, finger=True)
            ...
            for frame_idx, frame in tqdm(enumerate(container.decode(in_stream)), total=in_stream.frames, leave=False):
                ...
                if frame_idx < tasks[curr_task_idx]['frame_start']:
                    ...
                    img = draw_predefined_mask(img, color=(0,0,0), 
                        mirror=no_mirror, gripper=False, finger=False)
                    # resize
